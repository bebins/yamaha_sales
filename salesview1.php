...
<div class="container-box">
    ...
    <input type="date" id="fromDate" oninput="filterTableByDate()">
    <label for="toDate">To:</label>
    <input type="date" id="toDate" oninput="filterTableByDate()">
...
<script>
    function filterTableByPaymentMode() {
        ...
        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const paymentModeColumn = row.getElementsByTagName("td")[5]; // Assuming mode of payment is in the 10th column (index 9)
...
<script>
    function filterTableByBranch() {
        const selectedPaymentMode = document.getElementById("filterBranch").value.toLowerCase();
        const tableBody = document.getElementById("tableBody");
        const rows = tableBody.getElementsByTagName("tr");

        for (let i = 0; i < rows.length; i++) {
            const row = rows[i];
            const paymentModeColumn = row.getElementsByTagName("td")[6]; // Assuming mode of payment is in the 10th column (index 9)
...
function filterTable() {
    ...
    for (let i = 0; i < rows.length; i++) {
        const row = rows[i];
        const paymentModeColumn = row.getElementsByTagName("td")[5].textContent.toLowerCase();
        const branchColumn = row.getElementsByTagName("td")[6].textContent.toLowerCase();
